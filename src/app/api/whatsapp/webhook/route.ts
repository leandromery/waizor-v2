...
// Inbound processing lives in provider-agnostic modules now: the Meta
// normalizer turns this route's payload into the shared NormalizedInbound
// shape, and processInboundMessage owns everything downstream.
import {
  normalizeMetaMessage,
  type MetaWebhookMessage,
  type MetaWebhookContact,
} from '@/lib/whatsapp/inbound/meta-normalize'
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      for (let i = 0; i < value.messages.length; i++) {
        ...
        // Meta payload → shared NormalizedInbound (resolves media), then
        // hand off to the provider-agnostic processor.
        const normalized = await normalizeMetaMessage(
          message,
          contact,
